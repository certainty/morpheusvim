...
return {
  ...
  config = function()
    ...
    if Morpheus.is_enabled { 'lang', 'terraform', 'treesitter' } then
      table.insert(installed_parsers, 'terraform')
      utils.ftcmd('terraform', 'terraform', enable_treesitter)
    end

    if Morpheus.is_enabled { 'lang', 'html', 'treesitter' } then
      table.insert(installed_parsers, 'html')
      utils.ftcmd('html', 'html', enable_treesitter)
    end
